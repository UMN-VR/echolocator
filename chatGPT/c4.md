Okay, it is kind of working, but I can orbit regardless of where I click, I want to orbit only if I start my click on the orbit widget, which btw should have a border to be able to tell 




This is spectrogram.js
...
To ensure that orbiting only happens when the click starts within the orbit widget, and to add a border for visual delineation, certain modifications need to be implemented. Firstly, to add a border around the orbit widget visibly, you would typically use CSS if it were a separate HTML element. Since it's part of the canvas rendered by Three.js, you'll need to simulate this effect, perhaps by drawing a border using Three.js itself or adjusting the scissor test area slightly smaller than the viewport to create a border effect.

However, more importantly, you'll need a way to detect if the mouse down event starts within the widget area before allowing the orbit controls to interact. This would involve modifying how OrbitControls is enabled based on mouse position.

When I click anywhere on the page, I can see the x,y update on the top left when I release my click, but this is not the behavior I want, I want to create a sound using the x,y as params, and I want the sound to play continuously until I lift my finger, with the x,y params updating as I move the mouse (or finger, this needs to work on a touchscreen)

Please brainstorm what are the best ways to accomplish this, and then we can start implementing it together
